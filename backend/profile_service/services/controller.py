...
class ProfileController:
    ...
    async def get_profile(self, user_id: uuid.UUID) -> ProfileResponse:
        ...
        if profile.avatar_filename and not profile.avatar_filename.startswith(('http://', 'https://')):
            profile.avatar_filename = await self.file_service.get_avatar_url(profile.avatar_filename)
    ...
    async def get_all_profiles(self) -> List[ProfileResponse]:
        ...
        for profile in profiles:
            if profile.avatar_filename and not profile.avatar_filename.startswith(('http://', 'https://')):
                profile.avatar_filename = await self.file_service.get_avatar_url(profile.avatar_filename)
                await self.profile_service.update_avatar_url(profile.user_id, profile.avatar_filename)
